...
const mongoPwd = env.MONGO_PWD;
const serverIP = env.SERVER;
...
function dbConnectDisplayAccounts($arg = { useMongoClient: true }) {
  return new Promise(((resolve, reject) => {
    ...
    module.exports.dbConnect($arg)
      .then(() => dbCollections.accounts.listAll())
      .then(() => dbCollections.assets.listAll())
      .then(() => resolve())
      .catch(e => reject(e));
  }));
}
...
function recentAccounts(
  ...
  return new Promise(((resolve, reject) => {
    if (dbCollections) {
      if (typeof idFrom !== 'undefined' && idFrom !== '') {
        ...
        dbCollections.accounts.listRecent(idFrom)
          .then((ethAddressesArray) => {
            if (ethAddressesArray !== null && ethAddressesArray.length > 0) {
      ...
      module.exports.dbConnect($arg)
        .then(() => resolve(module.exports.recentAccounts()))
        .catch(e => reject(e));
    }
  }));
}
    ...
    if (dbCollections) {
      if (typeof idFrom !== 'undefined' && idFrom !== '') {
        ...
        dbCollections.assets.listRecent(idFrom)
          .then(assetsArray => resolve(assetsArray))
          .catch(e => reject(e));
      } else {
          ...
          .then((result) => {
            if (result.length === 0) {
              ethAddresses.addAddress(
                accountsArray[0].walletID,
                accountsArray[0].publicAddress, accountsArray[0].FCMIID
              );
            }
          }).catch(e => reject(e));
...
function resetDBTxHistory() {
  return new Promise(((resolve, reject) => {
    module.exports.emptyDBTxHistory()
      .then(() => module.exports.initDBTxHistory())
      .then(() => resolve())
      .catch(e => reject(e));
  }));
}
...
function resetDBERC20SmartContracts() {
  return new Promise(((resolve, reject) => {
    module.exports.emptyDBERC20SmartContracts()
      .then(() => module.exports.initDBERC20SmartContracts())
      .then(() => resolve())
      .catch(e => reject(e));
  }));
}
...
function emptyDB() {
  return new Promise(((resolve, reject) => {
    ...
    accounts.emptyCollection()
      .then(() => {
        const assets = require('../controllers/assets_ctrl.js');
        return assets.emptyCollection();
      })
      .then(() => {
        const transactions = require('../controllers/transactions_ctrl.js');
        return transactions.emptyCollection();
      })
      .then(() => {
...
function getTxHistory(address1, fromtmstmp, address2, asset) {
  return new Promise(((resolve, reject) => {
    ...
    transactions.getTxHistory(address1.toUpperCase(), fromtmstmp, address2.toUpperCase(), asset)
      .then(txHistory => resolve(txHistory))
      .catch(e => reject(e));
  }));
}
